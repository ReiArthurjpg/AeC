...
{ int UsuarioId=>int.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
 public async Task<IActionResult> Index(string? termo,string? ordenarPor,int pagina=1){ var data=await service.ListarAsync(UsuarioId,new(termo,ordenarPor,pagina,10)); var todos = await service.ListarTodosAsync(UsuarioId); int totalCidades = todos.Select(x => x.Cidade).Distinct(StringComparer.OrdinalIgnoreCase).Count(); return View(new EnderecoListViewModel{Enderecos=data,Termo=termo,OrdenarPor=ordenarPor, TotalCidades = totalCidades}); }
