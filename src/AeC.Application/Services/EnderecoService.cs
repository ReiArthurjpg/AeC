...
public sealed class EnderecoService(IEnderecoRepository repo) : IEnderecoService
{
    ...
    public async Task AtualizarAsync(int id,int usuarioId,EnderecoFormDto dto,CancellationToken ct=default){ var e=await repo.ObterDoUsuarioAsync(id,usuarioId,ct) ?? throw new DomainException("Endereço não encontrado."); Apply(e,dto); repo.Atualizar(e); await repo.SalvarAsync(ct); }
    ...
    static Endereco From(EnderecoFormDto d,int uid){ var e=new Endereco{UsuarioId=uid}; Apply(e,d); return e; }
    ...
    static EnderecoDto Map(Endereco e)=>new(e.Id,e.CEP,e.Logradouro,e.Complemento,e.Bairro,e.Cidade,e.UF,e.Numero,e.UsuarioId);
}
